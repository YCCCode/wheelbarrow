...
static void tft180_set_region (uint16 x1, uint16 y1, uint16 x2, uint16 y2)
{
    ...
    if(x1>tft180_x_max)x1=tft180_x_max-1;
    if(y1 > tft180_y_max)y1=tft180_y_max-1;
    if(x2 > tft180_x_max)x2=tft180_x_max-1;
    if(y2 > tft180_y_max)y2=tft180_y_max-1;
...
void tft180_draw_point (uint16 x, uint16 y, const uint16 color)
{
    ...
    TFT180_CS(0);
    tft180_set_region(93-x,60- y,93-x, 60-y);
    tft180_write_16bit_data(color);
    TFT180_CS(1);
}
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void tft180_show_binary_image (uint16 x, uint16 y, uint8 image[LCDH][LCDW], uint16 width, uint16 height, uint16 dis_width, uint16 dis_height)
{
    // 如果程序在输出了断言信息 并且提示出错位置在这里
    // 那么一般是屏幕显示的时候超过屏幕分辨率范围了
    zf_assert(x < tft180_x_max);
    ...
    TFT180_CS(0);
    tft180_set_region(x, y, x + dis_width/2 - 1, y + dis_height - 1);             // 设置显示区域
